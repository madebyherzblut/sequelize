...
module.exports = (function() {
  ...
  HasManyDoubleLinked.prototype.injectGetter = function(options) {
    ...
    var customEventEmitter = new Utils.CustomEventEmitter(function() {
      ...
      where[through.getTableName()+"."+self.association.identifier] = self.instance[instancePrimaryKey]
      ...
      where[through.getTableName()+"."+foreignKey] = {join: self.association.target.getTableName()+"."+foreignPrimary}
      ...
      if (Object(targetAssociation.through) === targetAssociation.through) {
        ...
        if (!options.attributes) {
          options.attributes = [self.association.target.getTableName()+".*"]
        }

        if (options.joinTableAttributes) {
          options.joinTableAttributes.forEach(function (elem) {
            options.attributes.push(
              through.getTableName() + '.' + self.QueryInterface.quoteIdentifiers(elem) + ' as ' +
          ...
          Utils._.forOwn(through.rawAttributes, function (elem, key) {
            options.attributes.push(
              through.getTableName() + '.' + self.QueryInterface.quoteIdentifiers(key) + ' as ' +
